...
class DeviceSpec(object):
    ...
    def process(self, data):
        ...
        for name, (chan, b1, b2, flip) in self.mappings.items():
            if data[0] == chan:
                value = flip * to_int16(data[b1], data[b2]) / float(self.axis_scale)

                # TODO #1
                # value = flip * to_int16(data[b1], data[b2]) / float(self.axis_scale)
                # self.dict_state[name] = value
        ...
        # TODO commented, because of TODO #1
...
device_specs = {
    "SpaceNavigator": DeviceSpec(name="SpaceNavigator",
                                 ...
                                 mappings={"REL_X": AxisSpec(channel=1, byte1=1, byte2=2, scale=1),
                                           "REL_Y": AxisSpec(channel=1, byte1=3, byte2=4, scale=-1),
                                           "REL_Z": AxisSpec(channel=1, byte1=5, byte2=6, scale=-1),
                                           "REL_RX": AxisSpec(channel=2, byte1=1, byte2=2, scale=-1),
                                           "REL_RY": AxisSpec(channel=2, byte1=3, byte2=4, scale=-1),
                                           "REL_RZ": AxisSpec(channel=2, byte1=5, byte2=6, scale=1),
                                           },
